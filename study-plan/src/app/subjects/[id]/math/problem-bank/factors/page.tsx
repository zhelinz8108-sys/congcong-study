...
import factorBankData from "@/lib/math-factor-bank.json";
...
type FactorBank = {
  ...
  questions: FactorQuestion[];
};

const factorBank = factorBankData as FactorBank;
...
function questionText(question: FactorQuestion) {
  return `${question.number} ${question.difficulty} ${question.category} ${question.prompt} ${question.answer} ${question.explanation}`;
}

export default function FactorProblemBankPage() {
  const { id } = useParams<{ id: string }>();
  const [query, setQuery] = useState("");
  const [difficulty, setDifficulty] = useState("全部难度");
  const [category, setCategory] = useState("全部题型");

  const difficulties = Object.keys(factorBank.difficultyCounts);
  const categories = Object.keys(factorBank.categoryCounts);

  const filteredQuestions = useMemo(() => {
    const keyword = query.trim().toLowerCase();
    return factorBank.questions.filter((question) => {
    ...
    <main className="min-h-screen bg-[#f6f8fb] px-4 py-6 text-neutral-950 sm:px-6">
      <div className="mx-auto max-w-6xl">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-5 text-xs font-semibold uppercase tracking-[0.22em] text-blue-500">
                Factor & Multiple Review
              </p>
              <h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">
                {factorBank.title}
              </h1>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-neutral-500">
                {factorBank.subtitle}。覆盖因数、倍数、质数合数、公因数公倍数和生活应用，适合电脑端集中复习。
              </p>
            </div>

            <div className="grid grid-cols-3 gap-2 text-center text-sm">
              <div className="rounded-xl bg-blue-50 px-4 py-3">
                <p className="text-lg font-bold text-blue-700">{factorBank.questionCount}</p>
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_180px_220px]">
            <label className="grid gap-1.5 text-sm font-semibold text-neutral-600">
              ...
              <input
                ...
                placeholder="例如：最大公因数、最小公倍数、质数、数字谜"
        ...
        <section className="mt-4 flex flex-wrap gap-2">
          {Object.entries(factorBank.difficultyCounts).map(([label, count]) => (
            <button
              key={label}
