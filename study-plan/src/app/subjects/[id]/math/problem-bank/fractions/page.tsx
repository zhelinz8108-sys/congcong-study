...
const fractionBank = fractionBankData as FractionBank;
...
function questionText(question: FractionQuestion) {
  return `${question.number} ${question.difficulty} ${question.category} ${question.prompt} ${question.answer} ${question.explanation}`;
}

export default function FractionProblemBankPage() {
  const { id } = useParams<{ id: string }>();
  const [query, setQuery] = useState("");
  const [difficulty, setDifficulty] = useState("全部难度");
  const [category, setCategory] = useState("全部题型");

  const difficulties = Object.keys(fractionBank.difficultyCounts);
  const categories = Object.keys(fractionBank.categoryCounts);

  const filteredQuestions = useMemo(() => {
    const keyword = query.trim().toLowerCase();
    return fractionBank.questions.filter((question) => {
    ...
    <main className="min-h-screen bg-[#f6f8fb] px-4 py-6 text-neutral-950 sm:px-6">
      <div className="mx-auto max-w-6xl">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-7 text-neutral-500">
                {fractionBank.subtitle}。题目、答案和详解已从 PDF 按左右栏整理成网页，适合大屏复习和检索。
              </p>
            </div>

            <div className="grid grid-cols-3 gap-2 text-center text-sm">
              <div className="rounded-xl bg-blue-50 px-4 py-3">
                <p className="text-lg font-bold text-blue-700">{fractionBank.questionCount}</p>
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_180px_220px]">
            <label className="grid gap-1.5 text-sm font-semibold text-neutral-600">
              ...
              <input
                ...
                placeholder="例如：通分、倒数、最小公倍数、108 米"
        ...
        <section className="mt-4 flex flex-wrap gap-2">
          {Object.entries(fractionBank.difficultyCounts).map(([label, count]) => (
            <button
              key={label}
